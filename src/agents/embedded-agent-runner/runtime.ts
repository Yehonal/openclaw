...
export function isOpenClawBuiltinAgentRuntime(runtime: string | undefined): boolean {
  return normalizeOptionalAgentRuntimeId(runtime) === OPENCLAW_AGENT_RUNTIME_ID;
}

/** @deprecated Compatibility for shipped config/session/env values that used the old Pi runtime id. */
export function isOpenClawBuiltinOrLegacyAgentRuntime(runtime: string | undefined): boolean {
  return normalizeOptionalLegacyAgentRuntimeId(runtime) === OPENCLAW_AGENT_RUNTIME_ID;
}
